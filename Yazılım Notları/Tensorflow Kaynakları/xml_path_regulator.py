...
def main():
    ...
    if args.prefix is not None:
        rename_files(args.inputDir, ['*jpg', '*xml'], args.prefix)

    regulate_xmls(args.inputDir)


def rename_files(pathname, patterns, prefix):
    for pattern in patterns:
        i = 1
        for file_path in glob.iglob(os.path.join(pathname, pattern)):
            filename, ext = os.path.splitext(os.path.basename(file_path))
            os.rename(file_path, os.path.join(pathname, prefix + ('%.5d' % i) + ext))
            i += 1
